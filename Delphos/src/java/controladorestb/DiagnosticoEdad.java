...
public class DiagnosticoEdad implements Control{
            ...
            throws javax.servlet.ServletException, java.io.IOException
    {
        ...
        if(request.getParameter("hacerSubmit") != null && !request.getParameter("hacerSubmit").equals("0"))
        {
            ...
            try
            {
                ...
                {
                    stmt = c.con.prepareStatement("INSERT INTO diagnosticoedad ( codunidmedidaedadfin, edadfinal, observacion, ususiscodigo, fecharegistro, coddiagnostico, codunidmedidaedadini, edadinicial ) values ( ?, ?, ?, ?, ?, ?, ?, ? )");
                    msj = "ingresada";                    

                }
                else if( request.getParameter("hacerSubmit").equals("2") )
                {
                    stmt = c.con.prepareStatement("UPDATE diagnosticoedad SET ( codunidmedidaedadfin, edadfinal, observacion, ususiscodigo, fecharegistro ) = ( ?, ?, ?, ?, ? ) WHERE coddiagnostico = ? and codunidmedidaedadini = ? and edadinicial = ?");
                    msj = "modificada";                                        
                }
                else
                {
                    stmt = c.con.prepareStatement("DELETE FROM diagnosticoedad WHERE coddiagnostico = ? and codunidmedidaedadini = ? and edadinicial = ?");
