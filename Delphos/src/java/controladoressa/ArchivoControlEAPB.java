...
public class ArchivoControlEAPB implements Control{
            ...
            throws javax.servlet.ServletException, java.io.IOException
    {
        ...
        if(request.getParameter("hacerSubmit") != null && !request.getParameter("hacerSubmit").equals("0"))
        {
            ...
            try
            {
                ...
                {
                    stmt = c.con.prepareStatement("INSERT INTO archivocontroleapb ( codtiporips, cantidadregistros, codestregrips, ususiscodigo, fecharegistro, idencabezado, idregistro) values ( ?, ?, ?, ?, ?, ?, ? )");
                    msj = "ingresada";                    
                                        
                }
                else if( request.getParameter("hacerSubmit").equals("2") )
                {
                    stmt = c.con.prepareStatement("UPDATE archivocontroleapb SET (codtiporips, cantidadregistros, codestregrips, ususiscodigo, fecharegistro) = ( ?, ?, ?, ?, ? ) WHERE idencabezado = ? and idregistro = ?");
                    msj = "modificada";                                        
                }
                else
                {
                    stmt = c.con.prepareStatement("DELETE FROM archivocontroleapb WHERE idencabezado = ? and idregistro = ?");                 
