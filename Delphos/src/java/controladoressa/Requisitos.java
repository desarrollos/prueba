...
public class Requisitos implements Control{
            ...
            throws javax.servlet.ServletException, java.io.IOException
    {
        ...
        if(request.getParameter("hacerSubmit") != null && !request.getParameter("hacerSubmit").equals("0"))
        {
            ...
            try
            {
                ...
                {
                    stmt = c.con.prepareStatement("INSERT INTO requisitocto ( aplicaa, observacion, archivo, archivofi, ususiscodigo, fecharegistro, codregimenes, codentidadsalud, codcontrato, codtiporequisito) values ( ?, ?, ?, "+archivoGuardar+", ?, ?, ?, ?, ?, ? )");
                    msj = "ingresada";                     
                    
                }
                else if( request.getParameter("hacerSubmit").equals("2") )
                {
                    stmt = c.con.prepareStatement("UPDATE requisitocto SET ( aplicaa, observacion, archivo, archivofi, ususiscodigo, fecharegistro ) = ( ?, ?, ?, "+archivoGuardar+", ?, ? ) WHERE codregimenes = ? and codentidadsalud = ? and codcontrato = ? and codtiporequisito = ? ");
                    msj = "modificada";                                        
                }
                else
                {
                    stmt = c.con.prepareStatement("DELETE FROM requisitocto WHERE codregimenes = ? and codentidadsalud = ? and codcontrato = ? and codtiporequisito = ?");
                    ...
                    stmt.setString(4,request.getParameter("codtiporequisitos"));
                ...
                if( !request.getParameter("hacerSubmit").equals("3") )
                {
                    ...
                    stmt.setString(9,request.getParameter("codtiporequisitos"));
