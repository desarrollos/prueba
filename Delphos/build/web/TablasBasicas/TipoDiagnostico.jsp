...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codtipodiagnostico','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array('codtipodiagnostico');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array('codtipodiagnostico');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM tipodiagnostico WHERE codtipodiagnostico='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codtipodiagnostico" value="${fila.codtipodiagnostico}"/>
			  <c:set var="descripcion" value="${fila.descripcion}"/>  
			  <c:set var="observacion" value="${fila.observacion}"/>   
            </jsp2:forEach>
                
                <table width="784" border="0" class="tablaContenido">
		<form action="TipoDiagnostico.jsp" method="post" name="formulario" id="formulario">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Tipo Diagn&oacute;stico </div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="112" class="bgDescTablas"><strong>C&oacute;digo Tipo Diagn&oacute;stico </strong>
                      <label></label></td>
                    <td width="546" class="bgDescTablas"><label>
                      <input name="codtipodiagnostico" type="text" id="codtipodiagnostico" value="${codtipodiagnostico}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/>
                    </label></td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codtipodiagnostico='+document.getElementById('codtipodiagnostico').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('tipodiagnostico',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=tipodiagnostico&";
        url = url + "campos=codtipodiagnostico,descripcion,observacion&";
        url = url + "campoLlave=codtipodiagnostico";
