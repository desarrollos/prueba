...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codprotocolo','descripcion');
            var combos = null;
			
            //validar
            var arrayNonulos               	= new Array('codprotocolo');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;			
            var arrayCadenasANCod          	= new Array('codprotocolo');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM protocoloactividad WHERE codprotocolo='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              ...
              %>
              <sql:query var="r" sql="select lo_export(protocoloactividad.archivofi, '${ruta}${archivo}') from protocoloactividad where codprotocolo='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
              <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <%------------------------------------------------------------%>
              </jsp2:forEach>
              <%------------------------------------------------------------%>
		...
		<form name="formulario" id="formulario" method="post" action="ProtocoloActividad.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="6"><div align="center" class="titulo1">Protocolo Actividad</div></td>
                  </tr>
                  <tr>
                    <td colspan="6">&nbsp;</td>
                  </tr>
                  <tr>
                    <td width="51" class="bgDescTablas"><label></label></td>
                    <td width="124" class="bgDescTablas"><strong>C&oacute;digo Protocolo</strong>
                      <label></label></td>
                    <td width="144" class="bgDescTablas">
                        <input name="codprotocolo" type="text" id="codprotocolo" value="${codprotocolo}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/>
                    </td>
                    <td width="25" class="bgDescTablas">&nbsp;</td>
                    <td width="68" class="bgDescTablas"><strong>Archivo</strong></td>
                    <td width="230" class="bgDescTablas"><span class="sinBgDescTablas" style="width: 20%">
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codprotocolo='+document.getElementById('codprotocolo').value;
        validarLlave('protocoloactividad',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=protocoloactividad&";
        url = url + "campos=codprotocolo,descripcion,archivo&";
        url = url + "campoLlave=codprotocolo";
