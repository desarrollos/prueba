...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codclaseremision','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array ('codclaseremision');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array ('codclaseremision');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM claseremision WHERE codclaseremision='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codclaseremision" value="${fila.codclaseremision}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="ClaseRemision.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Clase Remisi&oacute;n</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="137" class="bgDescTablas"><strong>C&oacute;digo Clase Remisi&oacute;n</strong>                      </td>
                    <td width="521" class="bgDescTablas"><input name="codclaseremision" type="text" id="codclaseremision" value="${codclaseremision}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codclaseremision='+document.getElementById('codclaseremision').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('claseremision',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=claseremision&";
        url = url + "campos=codclaseremision,descripcion,observacion&";
        url = url + "campoLlave=codclaseremision";
