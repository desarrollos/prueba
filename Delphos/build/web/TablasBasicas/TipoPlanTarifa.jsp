...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codtipoplantarifa','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array('codtipoplantarifa');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array('codtipoplantarifa');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM tipoplantarifa WHERE codtipoplantarifa='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codtipoplantarifa" value="${fila.codtipoplantarifa}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="800" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="TipoPlanTarifa.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Tipo Plan Tarifa</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <label><strong>Tipo Plan Tarifa</strong></label></td>
                    <td width="480" class="bgDescTablas"><input name="codtipoplantarifa" type="text" id="codtipoplantarifa" value="${codtipoplantarifa}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/>
                    <label></label></td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codtipoplantarifa='+document.getElementById('codtipoplantarifa').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('tipoplantarifa',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=tipoplantarifa&";
        url = url + "campos=codtipoplantarifa,descripcion,observacion&";
        url = url + "campoLlave=codtipoplantarifa";
