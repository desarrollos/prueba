...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codmarcavehiculo','descripcion','observacion');
            var combos = null;
			
            var arrayNonulos               	= new Array ('codmarcavehiculo');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;			
            var arrayCadenasANCod          	= new Array ('codmarcavehiculo');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM marcavehiculo WHERE codmarcavehiculo='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codmarcavehiculo" value="${fila.codmarcavehiculo}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="MarcaVehiculo.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Marca Vehiculo</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="137" class="bgDescTablas"><strong>C&oacute;digo </strong>
                    <label><strong>Marca Vehiculo</strong></label></td>
                    <td width="521" class="bgDescTablas"><input name="codmarcavehiculo" type="text" id="codmarcavehiculo" value="${codmarcavehiculo}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codmarcavehiculo='+document.getElementById('codmarcavehiculo').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('marcavehiculo',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=marcavehiculo&";
        url = url + "campos=codmarcavehiculo,descripcion,observacion&";
        url = url + "campoLlave=codmarcavehiculo";
