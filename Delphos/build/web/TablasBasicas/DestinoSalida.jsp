...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('coddestinosalida','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               = new Array('coddestinosalida');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM destinosalida WHERE coddestinosalida='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="coddestinosalida" value="${fila.coddestinosalida}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="DestinoSalida.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Destino a la Salida o Conducta Definida</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="521" class="bgDescTablas"><input name="coddestinosalida" type="text" id="coddestinosalida" value="${coddestinosalida}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'coddestinosalida='+document.getElementById('coddestinosalida').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('destinosalida',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=destinosalida&";
        url = url + "campos=coddestinosalida,descripcion,observacion&";
        url = url + "campoLlave=coddestinosalida";
