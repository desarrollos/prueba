...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codviaingreso','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array ('codviaingreso');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array ('codviaingreso');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM viaingreso WHERE codviaingreso='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codviaingreso" value="${fila.codviaingreso}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="ViaIngreso.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">V&iacute;a de Ingreso</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="132" class="bgDescTablas"><strong>C&oacute;digo V&iacute;a Ingreso</strong></td>
                    <td width="477" class="bgDescTablas"><input name="codviaingreso" type="text" id="codviaingreso" value="${codviaingreso}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codviaingreso='+document.getElementById('codviaingreso').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('viaingreso',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=viaingreso&";
        url = url + "campos=codviaingreso,descripcion,observacion&";
        url = url + "campoLlave=codviaingreso";
