...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codcausaincapacidad','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array ('codcausaincapacidad');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array ('codcausaincapacidad');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM causaincapacidad WHERE codcausaincapacidad='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codcausaincapacidad" value="${fila.codcausaincapacidad}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="CausaIncapacidad.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Causa Incapacidad</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="133" class="bgDescTablas"><strong>C&oacute;digo Causa Incapacidad</strong></td>
                    <td width="476" class="bgDescTablas"><input name="codcausaincapacidad" type="text" id="codcausaincapacidad" value="${codcausaincapacidad}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codcausaincapacidad='+document.getElementById('codcausaincapacidad').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('causaincapacidad',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=causaincapacidad&";
        url = url + "campos=codcausaincapacidad,descripcion,observacion&";
        url = url + "campoLlave=codcausaincapacidad";
