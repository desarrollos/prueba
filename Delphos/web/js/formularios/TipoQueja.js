$(document).ready( function( ) {
    ...
    $("#ajax").ajaxStop( function() {
       ...
       // alert( op + "  " + io );
        ...
        if( op == 'actualizar_tipoQueja' ) {
            if( es == 'si' && er == 'no' ) bottomMensaje('La queja se actualizo correctamente','Estado actualizacion', 'ex', 's');
            if( es == 'no' && er == 'si' ) bottomMensaje('La queja no registro correctamente: <br /><br /><span class="error"> ' 
                + ed + "</span>", 'Error al actualizar el registro', 'e', 'n');
        }
        
        if( op == 'eliminar_tipoQueja' ) {
            
            if( es == 'si' && er == 'no' ) bottomMensaje('La queja se borro correctamente', 'Estado borrado', 'ex', 's');
