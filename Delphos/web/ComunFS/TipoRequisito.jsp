...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codtiporequisito','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array ('codtiporequisito');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array ('codtiporequisito');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM tiporequisito WHERE codtiporequisito='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codtiporequisito" value="${fila.codtiporequisito}"/>
              <c:set var="descripcion" value="${fila.descripcion}"/>
              <c:set var="observacion" value="${fila.observacion}"/>          
            </jsp2:forEach>
                
                <table width="642" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="TipoRequisito.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="6"><div align="center" class="titulo1">Tipo Requisito Contrato</div></td>
                  </tr>
                  <tr>
                    <td colspan="6">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="127" class="bgDescTablas"><strong>C&oacute;digo Tipo Requisito</strong></td>
                    <td width="519" colspan="4" class="bgDescTablas"><label>
                      <input name="codtiporequisito" type="text" id="codtiporequisito" value="${codtiporequisito}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/>
                    </label></td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codtiporequisito='+document.getElementById('codtiporequisito').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('tiporequisito',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=tiporequisito&";
        url = url + "campos=codtiporequisito,descripcion,observacion&";
        url = url + "campoLlave=codtiporequisito";
