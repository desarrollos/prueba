...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codgradodiscapacidad','descripcion','observacion');
            var combos = null;
			
            var arrayNonulos               	= new Array ('codgradodiscapacidad');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;			
            var arrayCadenasANCod          	= new Array ('codgradodiscapacidad');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM gradodiscapacidad WHERE codgradodiscapacidad='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codgradodiscapacidad" value="${fila.codgradodiscapacidad}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="GradoDiscapacidad.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Grado Discapacidad</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="137" class="bgDescTablas"><strong>C&oacute;digo Grado Discapacidad</strong></td>
                    <td width="521" class="bgDescTablas"><input name="codgradodiscapacidad" type="text" id="codgradodiscapacidad" value="${codgradodiscapacidad}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codgradodiscapacidad='+document.getElementById('codgradodiscapacidad').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('gradodiscapacidad',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=gradodiscapacidad&";
        url = url + "campos=codgradodiscapacidad,descripcion,observacion&";
        url = url + "campoLlave=codgradodiscapacidad";
