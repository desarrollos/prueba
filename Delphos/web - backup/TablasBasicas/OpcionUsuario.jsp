...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = null;
            var combos = new Array('ususiscodigo','lisopccodigo','niseopcodigo');
			
            var arrayNonulos               = null;
            var arrayNumerosEnteros        = null;
            var arrayCombos                = new Array('ususiscodigo','lisopccodigo','niseopcodigo');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM opcionusuario WHERE ususiscodigo='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
		...
		<form name="formulario" id="formulario" method="post" action="OpcionUsuario.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="6"><div align="center" class="titulo1">Opciones Usuarios Sistema</div></td>
                  </tr>
                  <tr>
                    <td colspan="6">&nbsp;</td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                    <td width="245" class="bgDescTablas"><strong>Usuario</strong></td>
                    <td width="382" colspan="4" align="left" class="bgDescTablas">
                      <select name="ususiscodigo" id="ususiscodigo" style="width: 150px" tabindex="1">
                          <option id="ususiscodigo" value=""></option>
                          <sql:query var="r" sql="select ususiscodigo, ususisusuario from usuariosistema" dataSource="jdbc/delphos" scope="page"/>
                          <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
                           <c:choose>
                                <c:when test='${fila.ususiscodigo == ususiscodigo}'>
                                <option value="${fila.ususiscodigo}" selected="selected">${fila.ususisusuario}</option>
                                </c:when>
                            <c:otherwise>
                                <option value="${fila.ususiscodigo}">${fila.ususisusuario}</option>
                            </c:otherwise>
                           </c:choose>
                         </jsp2:forEach>
                      </select>
...
<script>
    ...
    function validarPK()
    {
        ...
        validarLlave('opcionusuario',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=opcionusuario&";
        url = url + "campos=ususiscodigo,lisopccodigo,niseopcodigo&";
