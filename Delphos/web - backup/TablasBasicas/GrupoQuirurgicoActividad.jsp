...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var combos = new Array('codgrupoquirurgico','codactividadsalud','codfactorgrupoquirurgico');
			
            var arrayNonulos               	= null;
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= new Array('codgrupoquirurgico','codactividadsalud','codfactorgrupoquirurgico');			
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM grupoquirurgicoactividad WHERE codgrupoquirurgico=${param.llave}" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              ...
              <c:set var="codfactorgrupoquirurgico" value="${fila.codfactorgrupoquirurgico}"/>   
              <c:set var="observacion" value="${fila.observacion}"/>       
            </jsp2:forEach>
                
                <table width="810" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="GrupoQuirurgicoActividad.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="6"><div align="center" class="titulo1">Grupo Quir&uacute;rgico</div></td>
                  </tr>
                  <tr>
                    <td colspan="6">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="138" class="bgDescTablas"><strong>C&oacute;digo Grupo Quir&uacute;rgico</strong>
                      <label></label></td>
                    <td width="165" class="bgDescTablas">
                    <select name="codgrupoquirurgico" id="codgrupoquirurgico" style="width: 150px" tabindex="1">
                    ...
                    <td width="129" class="bgDescTablas"><strong>Actividad Salud</strong></td>
                  ...
                  <tr>
                    ...
                    <td class="bgDescTablas"><strong>Factor Quir&uacute;rgico</strong></td>
                    <td colspan="4" class="bgDescTablas"><label>
                      <select name="codfactorgrupoquirurgico" id="codfactorgrupoquirurgico" style="width: 150px" tabindex="3">
                        <option id="codfactorgrupoquirurgico" value=""></option>
                        <sql:query var="r" sql="select codfactorgrupoquirurgico, descripcion from factorgrupoquirurgico" dataSource="jdbc/delphos" scope="page"/>
                        <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
                          <c:choose>
                            <c:when test='${fila.codfactorgrupoquirurgico == codfactorgrupoquirurgico}'>
                              <option value="${fila.codfactorgrupoquirurgico}" selected="selected">${fila.descripcion}</option>
                            </c:when>
                            <c:otherwise>
                              <option value="${fila.codfactorgrupoquirurgico}">${fila.descripcion}</option>
                            </c:otherwise>
                          </c:choose>
                        </jsp2:forEach>
                      </select>
...
<script>
    ...
    {      
        ...
        validarLlave('grupoquirurgicoactividad',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=grupoquirurgicoactividad&";
        url = url + "campos=codgrupoquirurgico,codactividadsalud,codfactorgrupoquirurgico,observacion&";
