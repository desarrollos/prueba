	...
	$("#ajax").ajaxStop( function() {
		 ...
		 if( op == 'registrar_documentocausal' ) {
		  ...
		  if( op == 'actualizar_documentocausal' ) {
			 if( es == 'si' && er == 'no' ) bottomMensaje('El Documento Causal se actualizo correctamente','Estado actualizacion', 'ex', 's');			 if( es == 'no' && er == 'si' ) bottomMensaje('El Documento Causal no registro correctamente: <br /><br /><span class="error"> '				  + ed + "</span>", 'Error al actualizar el registro', 'e', 'n'); 
		 } 
 
		 if( op == 'eliminar_documentocausal' ) {
			 if( es == 'si' && er == 'no' ) bottomMensaje('El Documento Causal se borro correctamente', 'Estado borrado', 'ex', 'n'); 
