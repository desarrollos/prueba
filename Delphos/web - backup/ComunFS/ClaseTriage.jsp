...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codclasetriage','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array ('codclasetriage');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array ('codclasetriage');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM clasetriage WHERE codclasetriage='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codclasetriage" value="${fila.codclasetriage}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="ClaseTriage.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Clasificaci&oacute;n Triage</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="151" class="bgDescTablas"><strong>C&oacute;digo
                    <label>Clasificaci&oacute;n Triage</label>
                    </strong></td>
                    <td width="458" class="bgDescTablas"><input name="codclasetriage" type="text" id="codclasetriage" value="${codclasetriage}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codclasetriage='+document.getElementById('codclasetriage').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('clasetriage',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=clasetriage&";
        url = url + "campos=codclasetriage,descripcion,observacion&";
        url = url + "campoLlave=codclasetriage";
