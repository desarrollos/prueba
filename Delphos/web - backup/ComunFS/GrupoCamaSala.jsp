...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codgrupocamasala','descripcion','observacion');
            var combos = null;
			
            var arrayNonulos               	= new Array ('codgrupocamasala');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;			
            var arrayCadenasANCod          	= new Array ('codgrupocamasala');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM grupocamasala WHERE codgrupocamasala='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codgrupocamasala" value="${fila.codgrupocamasala}"/>
              <c:set var="descripcion" value="${fila.descripcion}"/>
              <c:set var="observacion" value="${fila.observacion}"/>          
            </jsp2:forEach>
                
                <table width="642" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="GrupoCamaSala.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="6"><div align="center" class="titulo1">Grupo Cama o Sala</div></td>
                  </tr>
                  <tr>
                    <td colspan="6">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="102" class="bgDescTablas"><strong>C&oacute;digo Grupo</strong></td>
                    <td width="503" colspan="4" class="bgDescTablas"><label>
                      <input name="codgrupocamasala" type="text" id="codgrupocamasala" value="${codgrupocamasala}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/>
                    </label></td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codgrupocamasala='+document.getElementById('codgrupocamasala').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('grupocamasala',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=grupocamasala&";
        url = url + "campos=codgrupocamasala,descripcion,observacion&";
        url = url + "campoLlave=codgrupocamasala";
