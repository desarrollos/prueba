...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codclaseurgencia','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array ('codclaseurgencia');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array ('codclaseurgencia');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM claseurgencia WHERE codclaseurgencia='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codclaseurgencia" value="${fila.codclaseurgencia}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="ClaseUrgencia.jsp">
                  <span id="mensajeControlador">${msj}</span>
                  <tr>
                    <td colspan="3"><div align="center" class="titulo1">Clase Urgencia</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="137" class="bgDescTablas"><strong>C&oacute;digo Clase Urgencia</strong>                      </td>
                    <td width="521" class="bgDescTablas"><input name="codclaseurgencia" type="text" id="codclaseurgencia" value="${codclaseurgencia}" size="20" maxlength="6" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codclaseurgencia='+document.getElementById('codclaseurgencia').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('claseurgencia',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=claseurgencia&";
        url = url + "campos=codclaseurgencia,descripcion,observacion&";
        url = url + "campoLlave=codclaseurgencia";
