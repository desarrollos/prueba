...
<title>:: DELPHOS ::</title>
        ...
        <script type="text/javascript">
            ...
            var cajas = new Array('codcausaexterna','descripcion','observacion');
            var combos = null;
            
            //validar
            var arrayNonulos               	= new Array ('codcausaexterna');
            var arrayNumerosEnteros        	= null;
            var arrayCombos               	= null;
            var arrayCadenasANCod          	= new Array ('codcausaexterna');
...
<tr>
    ...
    <td valign="top">
            ...
            <sql:query var="r" sql="SELECT * FROM causaexterna WHERE codcausaexterna='${param.llave}'" dataSource="jdbc/delphos" scope="page"/>
            </c:if>
            <jsp2:forEach items="${r.rows}" var="fila" varStatus="contador">
              <c:set var="codcausaexterna" value="${fila.codcausaexterna}"/> 
              <c:set var="descripcion" value="${fila.descripcion}"/>   
              <c:set var="observacion" value="${fila.observacion}"/> 
            </jsp2:forEach>
                
                <table width="809" border="0" class="tablaContenido">
		<form name="formulario" id="formulario" method="post" action="CausaExterna.jsp">
                 <span id="mensajeControlador">${msj}</span>
                 <tr>
                    <td colspan="3"><div align="center" class="titulo1">Causas Externas</div></td>
                  </tr>
                  <tr>
                    <td colspan="3">&nbsp;</td>
                  </tr>
                  <tr>
                    ...
                    <td width="137" class="bgDescTablas"><strong>C&oacute;digo
                    <label>Causa Externa</label>
                    </strong></td>
                    <td width="521" class="bgDescTablas"><input name="codcausaexterna" type="text" id="codcausaexterna" value="${codcausaexterna}" size="20" maxlength="2" onkeypress="javascript:return permite(event, 'num_car_cod')" tabindex="1"/> </td>
                  </tr>
                  <tr>
...
<script>
    ...
    function validarPK()
    {
        llaves = 'codcausaexterna='+document.getElementById('codcausaexterna').value;
        //llaves+= 'codmunicipio='+document.getElementById('codmunicipio').value+',';
        validarLlave('causaexterna',llaves);
    } 
    ...
    function buscar()
    {
        ...
        url = url + "tabla=causaexterna&";
        url = url + "campos=codcausaexterna,descripcion,observacion&";
        url = url + "campoLlave=codcausaexterna";
